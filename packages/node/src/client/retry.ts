...
export function makeRetryable(axios: AxiosInstance, config: NormConfig) {
    axios.interceptors.request.use(axiosConfig => {
        if (axiosConfig.method && NON_IDEMPOTENT_METHODS.includes(axiosConfig.method)) {
            ...
            axiosConfig.headers[IDEMPOTENCY_KEY] = generateStandardId()
        }
    ...
    axiosRetry(axios, {
        ...
        onRetry(_retryCount, error, requestConfig) {
            if (
                ...
                requestConfig.headers[IDEMPOTENCY_KEY] = generateStandardId()
            }
        },
    });
}
